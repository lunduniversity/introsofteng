...
Follow these instructions to install the Robocode testing plugin and setup JUnit for system testing robots:
...
\item \textbf{Add JUnit as a library}. As in Lab 2, right click the Java project and select ``Build Path'' and click ``Add Libraries...'' and select JUnit. Click ``Next >'' followed by ``Finish'' to accept the default settings. 
...
\item Create a new run configuration for JUnit with RobotTestBed. FORTYDLIGA. Do this in the same way as when creating a normal JUnit run configuration, but you must also add -Drobocode.home=PATH TO ROBOCODE as an argument to the VM.
\end{enumerate}
