...
\end{center}
\item If time permits during Lab~2, uncomment the code snippet for ``wall avoidance'' in MovementSystem.java and move it according to the instructions. If you did not reach this point during the lab session, please do it before Lab~3 -- it is required to proceed with the automated system testing. Start Robocode and verify that BasicMeleeBot does not end up in any wall. \\\\If wall avoidance does not work, please pay attention to the red arrow in Figure~\ref{fig:classDiagram}. If you have copied the entire code snippet from the \#\#\#\#\#\#\# MOVEMENT \#\#\#\#\#\#\# section in BasicMeleeBot\_AntiPattern, then you have declared two new local variables with the same names in MovementSystem, i.e., these variables now \textit{shadow} the attributes -- make sure the update() method in MovementSystem indeed updates the attributes, not any local variables.
\end{enumerate}
