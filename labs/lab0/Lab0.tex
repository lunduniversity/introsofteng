...
\textbf{Task:} If you have never used the command-line interface before, it is now time to do so. 
\begin{enumerate}
\item Start the command-line interface. 
\begin{itemize}
...
\item Show your current position in the file system (if you got lost). Input the corresponding command below:
\begin{itemize}
\item[unix] \texttt{pwd} (print working directory) 
\item[Windows] \texttt{cd} (current directory)
\end{itemize}
\item Now list every file and directory in your current directory. Input the following:
\begin{itemize}
\item[unix] \texttt{ls} (list) 
\item[Windows] \texttt{dir}
\end{itemize}
\item Sometimes you need to also list the metadata of the files. Input the command again, but now with an additional option:
\begin{itemize}
\item[unix] \texttt{ls -l}
\item[Windows] \texttt{dir /a}
\end{itemize}
\item You can now continue navigating the tree structure of your file system. Move into a subdirectory by typing:
\begin{itemize}
\item[unix] \texttt{cd <Name of directory>} (change directory) 
\item[Windows] Same as above.
\end{itemize}
\item To navigate one level up in the tree structure, type the following:
\begin{itemize}
\item[unix] \texttt{cd ..}
\item[Windows] Same as above.
\end{itemize}
